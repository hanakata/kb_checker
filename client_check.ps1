...
foreach($server_info in $lines){
    ...
    Write-Output $client_kb_list

    $user_name = Get-WmiObject -ComputerName $server_ip -Class Win32_ComputerSystem -Property UserName -Credential $credential

    Write-Output $user_name.UserName
    Write-Output $os_name + $bit_value
    ...
    #     while($result.Read()){
    #         if($n -eq 1){
    #             $cve = '"JVN","' + $result[5] + '","' + $result[6] + '","' + $result[11] + '"'
    #             Write-Output $cve | Add-Content -Encoding utf8 $cve_filename
    #         }
    #         if($n -eq 2){
    #             $cve = '"NVD","' + $result[5] + '","' + $result[6] + '","' + $result[9] + '"'
    #             Write-Output $cve | Add-Content -Encoding utf8 $cve_filename
    #         }
    #         if($n -eq 3){
